"""Hamiltonian Monte Carlo for the 1D shock tube -- infer the 4 Sod
initial-condition parameters from final-time observations.

Gradient-based sibling of ``MCMC/mh_mcmc.py`` (random-walk Metropolis) and
``MCMC/vanilla_mcmc.py`` (fixed-step RWM) on the SAME inverse problem: sample the
posterior over

    m = [p_high, p_low, rho_high, rho_low]

given density observations at ``T_FINAL``. Instead of one small random-walk step,
HMC augments the 4 params with a momentum and proposes by simulating Hamiltonian
dynamics with a leapfrog integrator, so a single accepted proposal travels a
long, near-energy-conserving distance across the strongly anisotropic posterior
-- typically far higher effective sample size per forward solve than RWM, at the
cost of needing the log-posterior GRADIENT.
...
Gradient
--------
The forward models (exact Sod / Euler / GPR surrogate) are not autodifferentiable
here, so the gradient of the potential ``U(m) = -log pi(m | y)`` is taken by
finite differences in the normalized [0,1]^4 space:

    central  (``--fd-mode central``): 2*d forward solves / gradient  (accurate)
    forward  (``--fd-mode forward`` ): d   forward solves / gradient  (cheaper)

Because the observations sit in the flat regions (fronts excluded, ``--margin``),
the misfit is smooth in the params and these differences are well-behaved. Each
leapfrog trajectory costs ``L`` gradients, so HMC does a LOT of forward solves --
use ``--fast`` (in-process exact/euler) and start with ``--forward exact``.

Sampler details
---------------
* Normalized parameter space u in [0,1]^4 (the four scales p~1e5 vs rho~0.1 are
  made comparable).
* Uniform prior over the ``center * (1 -/+ frac)`` box (``--prior-frac``); inside
  the box the prior is constant (zero gradient), and the hard box constraint is
  enforced by REFLECTING the leapfrog trajectory at the unit-cube walls (position
  mirrored, momentum negated), keeping every proposal feasible.
* Diagonal mass matrix M; optionally adapted once from the warmup-chain variance
...
Run from the repo root::
    ...
    python MCMC/hmc_mcmc.py --fast
    python MCMC/hmc_mcmc.py --n-samples 600 --forward euler --fd-mode forward --fast
    python MCMC/hmc_mcmc.py --n-samples 1500 --L 30 --adapt-mass --fast
"""
...
from MCMC.mcmc_common import (  # noqa: E402
    ...
    build_observations, make_forward, make_plots, prior_bounds,
)
...
# sampler below only ever calls these on feasible u.
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# Effective sample size (for comparing mixing against the RW samplers).
# --------------------------------------------------------------------------- #
def ess_1d(x):
...
def main() -> None:
    ...
    # Observation methodology -- identical flags to the other samplers.
    ...
    ap.add_argument("--fast", action="store_true",
                    help="testing shortcut: run the exact/euler forward models "
                         "in-process instead of shelling out per solve (HMC does "
                         "many solves, so this is a big speedup)")
    args = ap.parse_args()
    ...
    if not args.no_plot:
        ...
        make_plots(res, outdir, label="hmc")
