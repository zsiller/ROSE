...
TRUTH operating point.

The problem constants, observation builder and forward models are imported from
the global ``helpers.inverse_common`` (shared with ``EnKF/`` and ``dakota_mcmc/``)
and re-exported here, so the samplers keep importing them from ``mcmc_common``.
This module then adds only the MCMC-specific pieces: the ensemble loader, the
uniform-prior box (``prior_bounds``) and the marginal/trace/field figure.
"""
...
# truth); re-exported so the samplers keep importing them from mcmc_common.
...
# --------------------------------------------------------------------------- #
# Prior: the padded uniform-prior box (the ensemble loader is shared upstream).
# --------------------------------------------------------------------------- #
