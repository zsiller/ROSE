#!/usr/bin/env python
"""Per-cell ensemble CRPS for the surrogate via the ``scores`` library.

Pushes a Dakota MCMC chain through the GPR surrogate at the external obs cells
(same pipeline as ``validation.py``) and scores the resulting predictive
ensemble against the observations with ``scores.probability.crps_for_ensemble``.

NOTE: the ensemble-CRPS estimator in ``scores.probability`` is named
``crps_for_ensemble`` (the bare ``crps_ensemble`` name lives in
``properscoring`` / ``xskillscore``). This script uses the ``scores`` one.

``scores`` is xarray-native and offers two ensemble estimators:
  * ``method="ecdf"``  -> exact CRPS of the empirical CDF
  * ``method="fair"``  -> unbiased E|X-y| - 1/2 E|X-X'|  (default here)

``crps_for_ensemble`` scores the raw ensemble against a point observation, so it
does NOT fold in the per-cell observation variance the way ``validation.py``'s
Gaussian-kernel path does.

Run from the repo root::

    python validation/crps_ensemble.py
"""
...
OUT = HERE / "surrogate_obs_dist_crps_ensemble.png"
CRPS_DAT = HERE / "surrogate_crps_ensemble.dat"
NO_PLOT = False


def crps_ensemble(pred: np.ndarray, obs: np.ndarray, cell_idx: np.ndarray,
                  method: str = CRPS_METHOD) -> np.ndarray:
    """Per-cell ensemble CRPS via scores.crps_for_ensemble. pred: (n_samples, m)."""
...
if __name__ == "__main__":
    ...
    print(f"[crps] backend=scores.crps_for_ensemble ({CRPS_METHOD}); "
          f"mean CRPS = {crps.mean():.4e} (lower is better; density units)")
    ...
    np.savetxt(
        ...
        fmt=["%d", "%.6f", "%.8e", "%.8e", "%.8e"],
        header=f"per-cell ensemble CRPS via scores ({CRPS_METHOD})  "
